...
```
L7  PRESENTATION    audit green + transport blue (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      openMCP
L4  AGENT MEMORY    agentmemory (v0.9.26) — shared mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: MCP server directory listings, audit scores, cross-post runs
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "openMCP"
...
```bash
git clone https://github.com/CSOAI-ORG/openMCP-hive
cd openMCP-hive
...
`https://openMCP/.well-known/agent-card.json` — see `agent-card.json`.
...
Free / OSS — drives traffic to monetised flagships.
